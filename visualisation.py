...
# Fonction pour sauvegarder, afficher la matrice, et prédire
def predict():
    global draw
    ...
    # Convertir en niveaux de gris et redimensionner en 20x20
    gray_image = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    resized_image = image_resize(gray_image, height=20)

    # Normaliser les valeurs pour correspondre au modèle (0 à 1)
    normalized_image = resized_image / 255.0

    # Afficher et sauvegarder la matrice redimensionnée
    print("Matrice utilisée pour la prédiction (20x20) :")
    print(normalized_image)
    np.savetxt("temp_image.txt", normalized_image, fmt='%.6f')  # Sauvegarde avec décimales
