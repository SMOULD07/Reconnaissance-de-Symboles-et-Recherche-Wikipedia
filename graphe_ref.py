...
def plot_references_graph(references):
    """
    Affiche un graphique des domaines des références externes trouvées.
    """
    # Extraire les domaines des références externes
    domains = [urlparse(ref).netloc for ref in references]
    ...
    plt.xlabel('Domaines des références')
    plt.ylabel('Nombre de références')
    plt.title('Distribution des références externes')
...
if __name__ == "__main__":
    ...
    if "error" in details:
        ...
        # Afficher les références externes sous forme de graphique
        print("\n=== Références externes ===")
        if details["external_links"]:
            plot_references_graph(details["external_links"])  # Afficher les références sous forme de graphique
        else:
            print("Aucune référence externe trouvée.")
